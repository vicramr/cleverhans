"""
Approximation of the SparseL1Descent attack. Instead of using exact gradients, NES
is used to approximate the gradients using only queries.
Paper link for NES: Paper link: https://proceedings.mlr.press/v80/ilyas18a.html
"""
import numpy as np
import torch

from cleverhans.torch.utils import zero_out_clipped_grads
from cleverhans.torch.utils import nes


def sparse_l1_descent_nes(
    model_fn,
    x,
    eps=10.0,
    eps_iter=1.0,
    ...
    n=50,
    sigma=0.001,
    #momentum=None, TODO also implement momentum
):
    """
    Parameters present in sparse_l1_descent are the same as in that function.
    ...
    for i in range(nb_iter):
        ...
        # Set up loss function for nes
        if targeted:
            loss_fn = lambda output, label: -criterion(output, label)
        else:
            loss_fn = criterion

        # Estimate gradient of loss with respect to input
        grad = nes(model_fn, adv_x, y, loss_fn, n, sigma)
